...
void DB::init()
{
    QDir collectionDBPath_dir(LIB::CollectionDBPath);
    if (!collectionDBPath_dir.exists())
        collectionDBPath_dir.mkpath(".");

    this->name = QUuid::createUuid().toString();
    if(!FMH::fileExists(LIB::CollectionDBPath + LIB::DBName))
    {
        this->openDB(this->name);
        qDebug()<<"Collection doesn't exists, trying to create it" << LIB::CollectionDBPath + LIB::DBName;
...
void DB::openDB(const QString &name)
{
    if(!QSqlDatabase::contains(name))
    {
        ...
        this->m_db.setDatabaseName(LIB::CollectionDBPath + LIB::DBName);
    }
